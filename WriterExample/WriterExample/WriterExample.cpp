...
extern "C" __declspec(dllexport) int32_t _stdcall MDF4AddChannelGroupToDataGroup(MDF4File * m4, M4DGBlock * dg, M_UINT64 rec_id, M4CGBlock * *cg);
extern "C" __declspec(dllexport) int32_t _stdcall MDF4AddChannelToChannelGroup(MDF4File * m4, M4DGBlock * dg, M4CGBlock * cg, char* channel_name, M_UINT64 id, M4CNBlock * *cn);
extern "C" __declspec(dllexport) int32_t _stdcall MDF4CreateDataStream(M4DGBlock * dg, M_UINT32 num_channel, M_UINT32 num_samples_per_read, m4DataStreamEx * *DTStream);
extern "C" __declspec(dllexport) int32_t _stdcall MDF4WriteStreamU64(m4DataStreamEx * DTStream, M_UINT32 channel_index, M_UINT64 * data);
extern "C" __declspec(dllexport) int32_t _stdcall MDF4WriteStreamDouble(m4DataStreamEx * DTStream, M_UINT32 channel_index, DOUBLE * data);
...
extern "C" __declspec(dllexport) int32_t _stdcall MDF4AddChannelGroupToDataGroup(MDF4File * m4, M4DGBlock * dg, M_UINT64 rec_id, M4CGBlock * *cg)
{
	...
	(*cg)->setRecordSize(sizeof(M_UINT64), 0UL);
	return 1;
}
extern "C" __declspec(dllexport) int32_t _stdcall MDF4AddChannelToChannelGroup(MDF4File * m4, M4DGBlock * dg, M4CGBlock * cg, char* channel_name, M_UINT64 id, M4CNBlock * *cn)
{
	...
	(*cn)->setLocationBytes(CN_D_UINT_LE, 0, sizeof(M_UINT64));
	(*cn)->Save();

	m4->addRecordCount(cg, id);
...
extern "C" __declspec(dllexport) int32_t _stdcall MDF4CreateDataStream(M4DGBlock * dg, M_UINT32 iterations, M_UINT32 num_samples_per_read, m4DataStreamEx * *DTStream)
{
	...
	(*DTStream) = new m4DataStreamEx(dg, uiBlkSize, iterations); //NO M_UINT32 dlEntries as parameter!!!
	return 1;
}
extern "C" __declspec(dllexport) int32_t _stdcall MDF4WriteStreamU64(m4DataStreamEx * DTStream, M_UINT32 channel_index, M_UINT64 * data)
{
	...
	if (DTStream->Write(sizeof(M_UINT64), data))// append
	{
		DTStream->AddRecords(1, channel_index); // group 1
		return 1;
	}
	return 0;
}
extern "C" __declspec(dllexport) int32_t _stdcall MDF4WriteStreamDouble(m4DataStreamEx * DTStream, M_UINT32 channel_index, DOUBLE * data)
{
	...
	//}
	//DTStream->AddRecords(uiNoOfRecords, channel_index); // group 1
	if (DTStream->Write(sizeof(DOUBLE), data))// append
	{
		DTStream->AddRecords(1, channel_index); // group 1
		return 1;
	}
